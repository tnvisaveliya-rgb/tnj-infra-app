...
export default function IssueReturnPage({ user }) {
  const [sitesList, setSitesList] = useState([]); // 🌟 plants ની જગ્યાએ sitesList
  ...
  const handleSubmitIssue = async (e) => {
    e.preventDefault();
    if (!selectedSite) {
      alert("⚠️ કૃપા કરીને પહેલા સાઇટ સિલેક્ટ કરો!");
      return;
    }
        ...
        for (const itm of src.items) {
         ...
         if (existingToolTx) {
            ...
            stockLedgerRows.push({
              ...
              qty: qtyNum,
              reference_id: existingToolTx.id,
              description: `Issued to ${person} (${itm.category || 'Tools'}) [Add-on]`
            });
      ...
      if (allTxToInsert.length > 0) {
        ...
        insertedTxs.forEach((row) => {
          stockLedgerRows.push({
            ...
            qty: row.quantity,
            reference_id: row.id,
            description: `Issued to ${row.issued_to} (${row.category})`
          });
        });
      }

      // 🌟 સાઇટ માટે હવે material_stock_ledger ની જગ્યાએ site_material_stock_ledger માં એન્ટ્રી પડશે
